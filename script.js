...
const display = (() => {
    ...
    // Initalize DOM variables that  will be used in multiple sections
    ...
    const createTask = (task, addToArray, priorityFlag) => {
        ...
        deletePng.setAttribute('src', 'img/delete.png');
        deleteContainer.appendChild(deletePng);
        taskDiv.appendChild(deleteContainer);
        // Priority on added task
        switch (priorityFlag) {
            case 'low':
                taskDiv.style.background = '#88FF88';
                break;
            case 'med':
                taskDiv.style.background = '#FFFF88';
                break;
            case 'high':
                taskDiv.style.background = '#FF7272';
        }
    ...
    const createList = (list, addToArray) => {
        ...
        if (list != 'Personal') {
            ...
            editList.setAttribute('src', 'img/edit.png')
            listOptions.appendChild(editList);
            const deleteList = document.createElement('img');
            deleteList.setAttribute('src', 'img/delete.png')
